...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === 'accept') {
    // Reinstate the vendor entirely and clear the suspension/appeal trail.
  ...
  // dismiss
  if (!response) {
    return NextResponse.json({ error: 'Please provide a response explaining why the appeal was dismissed.' }, { status: 400 });
  }
