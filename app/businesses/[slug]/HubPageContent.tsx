...
interface PreviewRequirement {
  ...
  necessity: string;
  image: string | null;
  // Only a valid link target when the source template is published — see
  // the requirementSlug computation in app/businesses/[slug]/page.tsx and
  // the US equivalent. Null means "don't render a link, show plain text."
...
function getSubPages(slug: string, base: string, market: MarketCode) {
  return [
    ...
    },
    {
      ...
      // US guides are deliberately deferred — the existing HowToGuide
      // content is Kenya-specific (eCitizen, county processes, KES
      // figures) and reusing it verbatim on the US market would be wrong,
      // the same reasoning already applied to business.faqs on the US hub
      // page. This route doesn't exist yet for the US market, so linking
      // to it unconditionally was a live broken link — see the Stage 0
      // fix notes.
...
>
  ...
  <span className="text-white/90 font-medium">{name}</span>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
                  ...
                  {previewRequirements.map((req) => (
                    <li key={req.id} className="flex items-center gap-4 px-6 py-4">
                      ...
                      <div className="flex-1 min-w-0">
                        {req.requirementSlug ? (
                          <Link
                            href={`/requirements/${req.requirementSlug}`}
                            className="text-sm font-medium text-gray-800 hover:text-emerald-700 hover:underline truncate block"
                          >
                            {req.name}
                          </Link>
                        ) : (
        ...
        {/* ── FAQ section ── */}
        <BusinessFaqSection name={name} faqs={faqs} />
