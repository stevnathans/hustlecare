...
type TradeClass = { id: number; name: string; slug: string };
...
export default function LegalFeeSchedulesAdminPage() {
  ...
  const [rowTradeClassId, setRowTradeClassId] = useState('');
  const [rowSizeBand, setRowSizeBand] = useState<SizeBand | ''>('');
  const [rowPrice, setRowPrice] = useState('');
  const [rowEmployeeMax, setRowEmployeeMax] = useState('');
  const [rowFloorArea, setRowFloorArea] = useState('');
    ...
    try {
      const r = await fetch('/api/admin/legal-fee-schedules', {
        ...
        body: JSON.stringify({
          ...
          tradeClassId: rowTradeClassId ? Number(rowTradeClassId) : null,
          sizeBand: rowSizeBand || null,
          price: Number(rowPrice),
          employeeCountMax: rowEmployeeMax ? Number(rowEmployeeMax) : null,
          floorAreaSqm: rowFloorArea ? Number(rowFloorArea) : null,
      ...
      if (!r.ok) throw new Error(d.error || 'Failed');
      showToast('Saved.');
      setRowCountyId(''); setRowTradeClassId(''); setRowSizeBand(''); setRowPrice('');
      setRowEmployeeMax(''); setRowFloorArea('');
      ...
      {selectedTemplate && (
        <>
          ...
          <div className="panel">
            ...
            <div className="panel-sub">
              Use this when one county's real fee is different from the flat rate above — or to price by
              trade class and/or size. Trade classes are how counties actually classify businesses for fees
              (e.g. "Hyper Supermarket") — not the site's browsing categories. Manage them via{' '}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: '0.85rem', marginBottom: '1rem' }}>
              ...
              <div>
                ...
                <select className="u-select" value={rowTradeClassId} onChange={(e) => setRowTradeClassId(e.target.value)}>
              ...
              <div>
                ...
                <input type="number" placeholder="e.g. 10" className="u-input" value={rowEmployeeMax} onChange={(e) => setRowEmployeeMax(e.target.value)} />
              </div>
              <div>
                <div className="f-label">Floor Area (sqm)</div>
                <input type="number" placeholder="e.g. 500" className="u-input" value={rowFloorArea} onChange={(e) => setRowFloorArea(e.target.value)} />
              </div>
              <div>
            ...
            <div style={{ fontSize: '0.72rem', color: '#55556e', marginBottom: '0.85rem', lineHeight: 1.5 }}>
              Max Employees and Floor Area are informational — shown to admins and on the county-fee display, but
              not used for automatic matching yet (that requires collecting the same data from businesses first).
              Automatic matching uses Trade Class + Size only.
            </div>
