...
const S = `
  ...
  .code-panel pre { padding:1rem; margin:0; font-family:'DM Mono',monospace; font-size:0.75rem; color:#9494b0; overflow-x:auto; line-height:1.7; }
  .code-panel pre .kw { color:#818cf8; }
  .code-panel pre .str { color:#34d399; }
  .code-panel pre .bool-t { color:#fbbf24; }
  .code-panel pre .bool-f { color:#f87171; }
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            ...
            <p style={{ fontSize: '0.84rem', color: '#55556e' }}>
              Manage URL redirects — {redirects.length} total
            </p>
          </div>
                  ...
                  <tr>
                    <td colSpan={7} style={{ textAlign: 'center', padding: '3.5rem', color: '#3a3a56' }}>
                      ...
                      <div style={{ color: '#55556e', fontWeight: 600 }}>
                        {search ? `No redirects match "${search}"` : 'No redirects yet'}
                      </div>
                      {!search && (
                        <button
                          onClick={openCreate}
                          style={{ marginTop: '0.75rem', fontSize: '0.8rem', color: '#818cf8', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', fontFamily: 'Sora,sans-serif' }}
                        >
                  ...
                  filtered.map(r => (
                    <tr key={r.id} style={{ background: selectedIds.includes(r.id) ? 'rgba(99,102,241,0.05)' : undefined }}>
                      ...
                      <td style={{ padding: '0.85rem 0.25rem' }}>
                        <ArrowRight size={13} color="#3a3a56" />
                      </td>
        ...
        {showCode && redirects.length > 0 && (
          <div className="code-panel">
            ...
            <pre dangerouslySetInnerHTML={{
              __html: generateCode()
                .replace(/\b(module|exports|async|return|const)\b/g, '<span class="kw">$1</span>')
                .replace(/'([^']+)'/g, '<span class="str">\'$1\'</span>')
                .replace(/\btrue\b/g, '<span class="bool-t">true</span>')
                .replace(/\bfalse\b/g, '<span class="bool-f">false</span>')
            }} />
          </div>
        )}
        {showCode && redirects.length === 0 && (
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  ...
                  <input
                    ...
                    autoFocus
                  />
                  <p style={{ fontSize: '0.72rem', color: '#55556e', marginTop: '0.35rem' }}>
                    Use <span style={{ color: '#9494b0' }}>:param</span> for dynamic segments, e.g. <span style={{ color: '#9494b0' }}>/blog/:slug</span>
                  </p>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.85rem', padding: '0.85rem 1rem', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10 }}>
                  <button
                    type="button"
                    className={`toggle-track ${formData.permanent ? 'on' : ''}`}
                    onClick={() => setFormData(p => ({ ...p, permanent: !p.permanent }))}
                    style={{ marginTop: '0.1rem' }}
                  >
                    <div className="toggle-thumb" />
                  </button>
                  <div>
                    <div style={{ fontSize: '0.84rem', fontWeight: 600, color: '#f0f0f5', marginBottom: '0.2rem' }}>
                      {formData.permanent ? '308 Permanent redirect' : '307 Temporary redirect'}
                    </div>
                    <div style={{ fontSize: '0.74rem', color: '#55556e' }}>
                      {formData.permanent
                        ? 'Browser caches forever. Best for SEO — tells search engines to update their index.'
                        : 'Browser re-checks each time. Use if this redirect might change.'}
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.65rem', marginTop: '0.5rem' }}>
                  ...
                  <button type="submit" disabled={loading} className="btn btn-primary">
                    {loading ? 'Saving…' : editing ? 'Save Changes' : 'Create Redirect'}
                  </button>
                </div>
              </form>
            </div>
