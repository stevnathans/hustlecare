...
export default function AdminVendorApplicationsPage() {
  ...
  const [filter, setFilter] = useState<
    "ALL" | "PENDING" | "APPROVED" | "REJECTED" | "UNCLAIMED" | "CLAIMED"
  >("PENDING");
  ...
  const [appealAction, setAppealAction] = useState<"accept" | "dismiss" | null>(
    null,
  );
  ...
  const [claimAction, setClaimAction] = useState<"approve" | "reject" | null>(
    null,
  );
  ...
  async function handleVendorAction() {
    ...
    try {
      const res = await fetch(
        `/api/admin/vendors/${actionModal.vendorId}/manage`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function handleAppealResolve() {
    ...
    try {
      const res = await fetch(
        `/api/admin/vendors/${appealModal.vendorId}/appeal`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      toast.success(
        appealAction === "accept"
          ? "Appeal accepted — vendor reinstated"
          : "Appeal dismissed",
      );
      setAppealModal(null);
  ...
  async function handleClaimResolve() {
    ...
    try {
      const res = await fetch(
        `/api/admin/vendors/${claimModal.vendorId}/claim`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      toast.success(
        claimAction === "approve"
          ? "Claim approved — vendor transferred"
          : "Claim rejected",
      );
      setClaimModal(null);
      setClaimAction(null);
    ...
    <div style={S.page}>
      ...
      <div style={S.header}>
        <div>
          ...
          <p style={S.subtitle}>
            Review, approve, and manage vendors on the Hustlecare marketplace
          </p>
        </div>
      ...
      {pendingAppeals.length > 0 && (
        <div style={S.appealsStrip}>
          <div style={S.appealsStripHead}>
            ...
            <span>
              {pendingAppeals.length} suspension appeal
              {pendingAppeals.length > 1 ? "s" : ""} awaiting review
            </span>
          </div>
          <div
                    ...
                    <div style={S.appealQuote}>
                      {a.vendor.issueResolved && (
                        <span style={S.resolvedTag}>
                          <CheckSquare size={11} /> Vendor marked issue resolved
                        </span>
                      )}
                      &ldquo;{a.vendor.appealMessage}&rdquo;
                      {a.vendor.appealedAt && (
                        <span style={{ color: "#3a3a56" }}>
                          {" "}
                          — {formatDate(a.vendor.appealedAt)}
                        </span>
                      )}
                    </div>
                  )}
                  ...
                  <button
                    ...
                    onClick={() => {
                      setAppealModal({
                        ...
                        vendorName: a.businessName,
                        suspendReason: a.vendor?.suspendReason ?? null,
                        appealMessage: a.vendor?.appealMessage ?? null,
                        issueResolved: a.vendor?.issueResolved,
                        appealedAt: a.vendor?.appealedAt ?? null,
                      });
      ...
      {pendingClaims.length > 0 && (
        ...
        >
          <div style={{ ...S.appealsStripHead, color: "#34d399" }}>
            ...
            <span>
              {pendingClaims.length} vendor claim
              {pendingClaims.length > 1 ? "s" : ""} awaiting review
            </span>
          </div>
          <div
            style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}
            ...
            {pendingClaims.map((a) => (
              <div key={a.id} style={S.appealRow}>
                <div style={{ minWidth: 0, flex: 1 }}>
                  ...
                  >
                    Claimed by: {a.vendor?.claimRequestedBy?.name ?? "—"} (
                    {a.vendor?.claimRequestedBy?.email ?? "—"})
                  </div>
                  {a.vendor?.claimMessage && (
                    <div style={S.appealQuote}>
                      &ldquo;{a.vendor.claimMessage}&rdquo;
                    </div>
                  )}
                </div>
                <div style={{ display: "flex", gap: "0.4rem", flexShrink: 0 }}>
      ...
      <div style={S.tableWrap}>
        {loading ? (
          <div style={S.loadCenter}>
            <Loader2
              size={22}
              style={{ animation: "spin 1s linear infinite", color: "#55556e" }}
            />
          </div>
        ) : filtered.length === 0 ? (
          <div style={S.emptyState}>
            <Store
              size={28}
              style={{ color: "#3a3a56", marginBottom: "0.6rem" }}
            />
                ...
                return (
                  <>
                    ...
                    >
                      ...
                      <td style={S.td}>
                        ...
                        >
                          ...
                          {app.productCategories.length === 0 && (
                            <span
                              style={{ color: "#3a3a56", fontSize: "0.78rem" }}
                            >
                              —
                            </span>
                          )}
                        ...
                        {app.vendor ? (
                          ...
                          >
                            <Package
                              size={13}
                              style={{
                                color: productCount > 0 ? "#818cf8" : "#3a3a56",
                              }}
                            />
                            <span
                              style={{
                            ...
                            {vendorStatus && (
                              <span
                                style={{
                                  ...S.vendorStatusBadge,
                                  background:
                                    VENDOR_STATUS_META[vendorStatus]?.bg ??
                                    "rgba(255,255,255,0.05)",
                                  color:
                                    VENDOR_STATUS_META[vendorStatus]?.color ??
                                    "#9494b0",
                                }}
                              >
                                {VENDOR_STATUS_META[vendorStatus]?.label ??
                                  vendorStatus}
                              </span>
                            )}
                      ...
                      {/* Actions */}
                      <td
                        style={{ ...S.td, textAlign: "right" as const }}
                      ...
                      >
                        ...
                        >
                          ...
                          {canManageVendor && (
                            <>
                              ...
                              <button
                                style={S.iconBtn}
                                title="Edit vendor"
                                onClick={() => openEditVendor(app.vendor!)}
                              >
                                <Pencil size={13} />
                              </button>
                          ...
                          <button
                            style={{ ...S.iconBtn, marginLeft: "0.1rem" }}
                          >
                            {isOpen ? (
                              <ChevronUp size={13} />
                            ) : (
                                  ...
                                  {appealStatus && appealStatus !== "NONE" && (
                                    <div style={{ marginTop: "0.5rem" }}>
                                      ...
                                      {app.vendor?.appealMessage && (
                                        ...
                                        >
                                          &ldquo;{app.vendor.appealMessage}
                                          &rdquo;
                                        </p>
                                      )}
                                      {appealStatus === "PENDING" && (
                                            ...
                                            onClick={() => {
                                              setAppealModal({
                                                ...
                                                appealMessage:
                                                  ...
                                                  null,
                                                issueResolved:
                                                  app.vendor?.issueResolved,
                                                appealedAt:
                                                  app.vendor?.appealedAt ??
                                                  null,
                                              });
                                              setAppealAction("accept");
                                              setAppealResponse("");
                              ...
                              {claimStatus && claimStatus !== "NONE" && (
                                <div>
                                  ...
                                  <p style={{ ...S.detailText }}>
                                    {app.vendor?.claimRequestedBy?.name ??
                                      "Unknown"}{" "}
                                    (
                                    {app.vendor?.claimRequestedBy?.email ?? "—"}
                                    )
                                  </p>
        ...
        <div style={S.tableFooter}>
          Showing{" "}
          <strong style={{ color: "#9494b0" }}>{filtered.length}</strong> of{" "}
          <strong style={{ color: "#9494b0" }}>{applications.length}</strong>{" "}
          applications
        </div>
      </div>
          ...
          <div style={S.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: "1.25rem" }}>
              <label style={S.modalLabel}>
                Note to applicant{" "}
                <span style={{ fontWeight: 400, color: "#3a3a56" }}>
                  (optional)
                </span>
              </label>
              <textarea
            ...
            >
              <button style={S.cancelBtn} onClick={() => setReviewModal(null)}>
                Cancel
              </button>
              <button
                style={{
        ...
        <div style={S.overlay} onClick={() => setActionModal(null)}>
          <div style={S.modal} onClick={(e) => e.stopPropagation()}>
            <div style={{ textAlign: "center", marginBottom: "1.25rem" }}>
              ...
              >
                {actionModal.action === "delete" &&
                  `Delete "${actionModal.vendorName}"?`}
                {actionModal.action === "suspend" &&
                  `Suspend "${actionModal.vendorName}"?`}
                {actionModal.action === "unsuspend" &&
                  `Reinstate "${actionModal.vendorName}"?`}
              </h3>
              <p
                style={{
                  fontSize: "0.8rem",
              ...
              >
                {actionModal.action === "delete" &&
                  "The vendor profile will be permanently removed. Their products will be archived. This cannot be undone."}
                {actionModal.action === "suspend" &&
                  "The vendor's storefront and products will be hidden from the marketplace. They cannot add new products."}
                {actionModal.action === "unsuspend" &&
                  "The vendor's storefront and active products will be restored to the marketplace."}
              </p>
            </div>
            ...
            >
              <button style={S.cancelBtn} onClick={() => setActionModal(null)}>
                Cancel
              </button>
              <button
                style={{
                ...
                disabled={
                  actionSubmitting ||
                  (actionModal.action === "suspend" && !actionNote.trim())
                }
              >
                {actionSubmitting && (
                  <Loader2
        ...
        >
          <div style={S.modal} onClick={(e) => e.stopPropagation()}>
            <div style={{ textAlign: "center", marginBottom: "1.25rem" }}>
              ...
              >
                {appealAction === "accept"
                  ? `Reinstate "${appealModal.vendorName}"?`
                  : `Dismiss appeal from "${appealModal.vendorName}"?`}
              </h3>
              <p
                style={{
                  fontSize: "0.8rem",
            ...
            <div style={S.appealContext}>
              ...
              <p style={{ ...S.detailText, fontStyle: "italic" }}>
                &ldquo;{appealModal.appealMessage ?? "—"}&rdquo;
              </p>
              {appealModal.appealedAt && (
                ...
                >
                  Submitted {formatDate(appealModal.appealedAt)}
                </p>
              )}
            </div>
            ...
            >
              ...
              <button
                ...
                disabled={
                  appealSubmitting ||
                  (appealAction === "dismiss" && !appealResponse.trim())
                }
              >
                {appealSubmitting && (
                  <Loader2
          ...
          <div style={S.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ ...S.appealContext, marginTop: "0.85rem" }}>
              ...
              <p style={{ ...S.detailText, marginBottom: "0.6rem" }}>
                ...
                {claimModal.claimantPhone
                  ? ` · ${claimModal.claimantPhone}`
                  : ""}
              </p>
              <div style={S.detailLabel}>Message</div>
              <p style={{ ...S.detailText, fontStyle: "italic" }}>
                &ldquo;{claimModal.claimMessage ?? "—"}&rdquo;
              </p>
            </div>
            ...
            >
              ...
              <button
                ...
                disabled={
                  claimSubmitting ||
                  (claimAction === "reject" && !claimRejectionReason.trim())
                }
              >
                {claimSubmitting && (
                  <Loader2
      ...
      {vendorFormOpen && (
        <VendorFormModal
          ...
          onSaved={() => {
            ...
            fetchApplications();
            toast.success(
              editingVendor ? "Vendor updated" : "Vendor created and live",
            );
          }}
        />
      )}
...
function ClaimCrossCheck({
  ...
  const domainMatch =
    !!vendorDomain && !!claimantDomain && vendorDomain === claimantDomain;
  const domainKnown = !!vendorDomain;

  const phoneMatch =
    !!vendorPhone &&
    !!claimantPhone &&
    normalizePhone(vendorPhone) === normalizePhone(claimantPhone);
  ...
  return (
    <div style={S.crossCheckBox}>
      ...
      >
        This is a heuristic, not proof — a mismatch doesn&rsquo;t mean the claim
        is false, and a match doesn&rsquo;t guarantee it&rsquo;s genuine. Use it
        alongside the claimant&rsquo;s message and, ideally, a direct check via
        the vendor&rsquo;s phone number on file.
      </p>
    </div>
  );
}
      ...
      >
        ...
        >
          ...
          <div style={{ marginBottom: "0.9rem" }}>
            <label style={S.modalLabel}>Market</label>
            <select
          ...
          <div style={{ marginBottom: "0.9rem" }}>
            ...
            <input
              style={S.modalInput}
              placeholder="e.g. Quality tools at fair prices"
              maxLength={120}
              value={form.tagline}
              onChange={(e) =>
