...
export default function Menu() {
  ...
  return (
    // Both the desktop and mobile <NotificationBell/> below are always
    // mounted (Tailwind's hidden/lg:flex only hides with CSS), so without
    // this shared provider each one ran its own independent fetch AND its
    // own 60s polling interval — doubling /api/user/notifications traffic
    // for the whole session. Wrapping here means both share one fetch/poll.
