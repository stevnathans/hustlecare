...
import { DollarSign, Clock, TrendingUp, Wrench, ExternalLink } from 'lucide-react';
...
export default function EnhancedBusinessCard({
  ...
  const totalRequirements = useMemo(
    () => Object.values(groupedRequirements).reduce((sum, reqs) => sum + (reqs?.length ?? 0), 0),
    [groupedRequirements],
  );
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          <table className="w-full border-collapse">
            <tbody>
              ...
              <Row
                icon={<FiFileText className="w-3.5 h-3.5 text-emerald-600 flex-shrink-0" />}
                label="Requirements"
              >
              ...
              {/* Startup cost */}
              <Row
                icon={<DollarSign className="w-3.5 h-3.5 text-blue-500 flex-shrink-0" />}
                label="Startup Cost"
                skeleton={costLoading}
              >
                {!costLoading && (
                  cost?.hasPricing
                    ? <>{formatKES(cost.low)}<span className="text-gray-400 font-normal"> – </span>{formatKES(cost.high)}</>
                    : <span className="text-gray-400 font-normal text-sm">Not available</span>
                )}
              </Row>
              ...
              {hasTime && (
                <Row
                  icon={<Clock className="w-3.5 h-3.5 text-sky-500 flex-shrink-0" />}
              ...
              {profitCfg && (
                <Row
                  icon={<TrendingUp className="w-3.5 h-3.5 text-lime-600 flex-shrink-0" />}
              ...
              {skillCfg && (
                <Row
                  icon={<Wrench className="w-3.5 h-3.5 text-violet-500 flex-shrink-0" />}
                  label="Skill Level"
                >
                  {skillCfg.label}
